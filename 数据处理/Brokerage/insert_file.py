...
def generate_sql(table_name, value):
    ...
    if table_name == 'shenzhen_month':
        ...
        sql = "INSERT INTO " + table_name + "(single_day_situation, stock, mainboard_A, mainboard_B, ipo," \
                                            ...
                                            " %s, %s, %s, %s, %s, %s, %s" \
                                            " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + value[-1] + "' and stock = '" + \
              value[1] + "')"
        ...
        sql = "INSERT INTO " + table_name + "(single_day_situation, fund, closed_fund, ETF, LOF," \
                                            ...
                                            " %s, %s, %s, %s, %s, %s, %s, %s" \
                                            " FROM DUAL WHERE NOT EXISTS(SELECT * FROM " + table_name + " WHERE `current_time` = '" + value[-1] + "' and fund = '" + \
              value[1] + "')"
    return sql
...
if __name__ == '__main__':
    path_db = { '深交所普通专区市场总貌': 'shenzhen_total_day',
                '上交所股票成交概况': 'sse_stock_day',
                '上交所基金成交概况': 'sse_fund_day'
               }
